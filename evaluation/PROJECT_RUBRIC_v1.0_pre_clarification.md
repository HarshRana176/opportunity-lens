# Phase 2 Project-Aware Relevance Rubric — v1.0 (FROZEN)

**Status: frozen before labelling.** This rubric must not be modified after the blinded worksheet is created or after any label is observed. Any future revision must be a new, separately versioned document (v1.1, v2.0, ...), never an in-place edit of this one, so that every label ever assigned can be traced to the exact rubric version it was assigned under.
